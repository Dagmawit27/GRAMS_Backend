...
public class RentReminderScheduler {
    ...
    /**
     * Calculate the next due date based on the payment due day.
     * If today is past this month's due day, next due is next month's due day.
     */
    private LocalDate calculateNextDueDate(LocalDate today, int dueDay) {
        ...
        // If this month's due date has passed (more than 5 days ago), look at next month
        // But we still want to show overdue for current month, so only advance if paid
        // Actually, just return this month's due date if it hasn't passed by more than 30 days
        if (today.isAfter(thisMonthDue.plusDays(30))) {
            // Way past due, use next month
