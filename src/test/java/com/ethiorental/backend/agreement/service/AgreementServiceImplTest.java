...
class AgreementServiceImplTest {
    ...
    @Test
    void createAgreementFromLeaseRequest_MultiUnit_OtherUnitsAvailable_BuildingRemainsListed() {
        UUID propertyId = UUID.randomUUID();
        Property property = Property.builder()
                .id(propertyId)
                .propertyCode("MALL-001")
                .title("Bole Shopping Mall")
                .status(PropertyStatus.LISTED)
                .build();

        PropertyUnit unitToRent = PropertyUnit.builder()
                .id(UUID.randomUUID())
                .unitCode("SHOP-101")
                .status(UnitStatus.AVAILABLE)
                .build();

        PropertyUnit remainingAvailableUnit = PropertyUnit.builder()
                .id(UUID.randomUUID())
                .unitCode("SHOP-102")
                .status(UnitStatus.AVAILABLE)
                .build();

        Citizen tenant = Citizen.builder().firstName("Abebe").lastName("Haile").email("abebe@example.com").build();
        Citizen landlord = Citizen.builder().email("landlord@example.com").build();
        ...
        LeaseRequest leaseRequest = LeaseRequest.builder()
                ...
                .applicant(tenant)
                .landlord(landlord)
                .proposedRent(BigDecimal.valueOf(25000))
                .landlordSigned(true)
                .tenantSigned(true)
                .build();

        when(agreementRepository.findByRequestCode("LR-MALL-1")).thenReturn(Optional.empty());
        when(agreementRepository.save(any(Agreement.class))).thenAnswer(i -> i.getArgument(0));
        when(propertyUnitRepository.findByPropertyId(propertyId))
                .thenReturn(List.of(unitToRent, remainingAvailableUnit));

        AgreementResponse response = agreementService.createAgreementFromLeaseRequest(leaseRequest, "supervisor@addis.gov.et");

        assertThat(response).isNotNull();
        assertThat(unitToRent.getStatus()).isEqualTo(UnitStatus.RENTED);
        verify(propertyUnitRepository).save(unitToRent);

        // Building should remain LISTED because SHOP-102 is still AVAILABLE
        verify(propertyRepository).save(property);
        assertThat(property.getStatus()).isEqualTo(PropertyStatus.LISTED);
    }

    @Test
    void createAgreementFromLeaseRequest_SingleHouse_PropertyMarkedRented() {
        Property property = Property.builder()
                .id(UUID.randomUUID())
                .propertyCode("HOUSE-001")
                .title("Single Family Villa")
                .status(PropertyStatus.LISTED)
                .build();

        Citizen tenant = Citizen.builder().email("abebe@example.com").build();
        Citizen landlord = Citizen.builder().email("landlord@example.com").build();

        LeaseRequest leaseRequest = LeaseRequest.builder()
                .id(UUID.randomUUID())
                .requestCode("LR-VILLA-1")
                .property(property)
                .unit(null) // No unit
                .applicant(tenant)
                .landlord(landlord)
                .proposedRent(BigDecimal.valueOf(35000))
                .landlordSigned(true)
                .tenantSigned(true)
                .build();

        when(agreementRepository.findByRequestCode("LR-VILLA-1")).thenReturn(Optional.empty());
        when(agreementRepository.save(any(Agreement.class))).thenAnswer(i -> i.getArgument(0));
